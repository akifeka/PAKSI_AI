...
# ====================================
# DETEKSI TREN & PREDIKSI AI
# ====================================

st.subheader("Analisis Tren dan Prediksi IKSI")

# Data tahun dan IKSI
X = tren_di["Tahun"].astype(int).values.reshape(-1,1)

y = tren_di["IKSI"].values
...
# Mengambil arah tren historis
slope = model.coef_[0]

# ====================================
# MODEL DEGRADASI SISTEM IRIGASI
# ====================================

tahun_prediksi = [2026, 2027, 2028]

# Nilai terakhir aktual
nilai_awal = y[-1]

# Faktor degradasi alami
degradasi = 2

prediksi = []

nilai_prediksi = nilai_awal

for tahun in tahun_prediksi:

    # Jika tren historis turun,
    # gunakan slope historis + degradasi

    if slope < 0:

        nilai_prediksi = nilai_prediksi + slope - degradasi

    # Jika tren historis naik/stabil,
    # tetap mengalami degradasi alami

    else:

        nilai_prediksi = nilai_prediksi - degradasi

    prediksi.append(round(nilai_prediksi,2))
...
st.write("Tren IKSI :", tren)

st.write("Perubahan rata-rata per tahun :", round(slope,2))
...
prediksi_df = pd.DataFrame({
    ...
    "Prediksi IKSI": prediksi
})

st.subheader("Prediksi IKSI Jika Tidak Ada Penanganan")
...
nilai_terakhir = prediksi[-1]

if nilai_terakhir < 55:

    st.error(
        "⚠ Diperkirakan masuk kategori "
        "'Jelek dan Perlu Perhatian' "
        "apabila tidak dilakukan penanganan."
    )

elif nilai_terakhir < 70:

    st.warning(
        "⚠ Diperkirakan mengalami "
        "penurunan kinerja dan perlu perhatian."
    )

else:

    st.success(
        "Kinerja diperkirakan masih stabil."
    )
    # ====================================
# KATEGORI KINERJA BERDASARKAN PAKSI
# ====================================

st.subheader("Kategori Kinerja Sistem Irigasi")

nilai_masa_depan = prediksi[-1]

# Kategori berdasarkan PAKSI

if nilai_masa_depan >= 80:

    kategori = "🟢 Kinerja Sangat Baik"

elif nilai_masa_depan >= 70:

    kategori = "🔵 Kinerja Baik"

elif nilai_masa_depan >= 55:
